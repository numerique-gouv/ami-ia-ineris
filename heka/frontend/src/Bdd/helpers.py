...
def uplaod_file(mzml_name, mzml_content):
    ...
    data = requests.post(url='https://heka-dev.sia-partners.ai/ineris/api/a12n/login',verify=False,data = {'username': username,'password': password})
    token=data.json()['access_token']
    uri = "https://heka-dev.sia-partners.ai/ineris/api/launcher/tasks/4/processes/async"
    response = requests.post(uri,verify=False,headers={'Authorization': 'Bearer {token}'.format(token=token)})
...
data = '{"arguments":["test.mzML"],"environment_variables":[{}]}'

response = requests.post('https://heka-dev.sia-partners.ai/ineris/api/launcher/tasks/4/processes/async', headers=headers, data=data)
