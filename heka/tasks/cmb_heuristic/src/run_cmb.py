...
def get_profiles(list_profils):
    """
    Get means and std of profiles from db
    profiles : is a list of profile names.
    """

    str_profils = str(list_profils).strip().replace('[', '(').replace(']', ')')
    query_profils = f"""
        SELECT *
        FROM public.colorado_profiles
        WHERE name IN {str_profils}
        ORDER BY name, amus
    """

    connection = psycopg2.connect(user = USER, password = PASSWORD, host = HOSTNAME, port = PORT, database = DATABASE)
    df = sqlio.read_sql_query(query_profils, connection)
    connection.close()

    df_final = pd.DataFrame(columns=['mass'])
    df_final['mass'] = pd.unique(df['amus'])

    for pro in list_profils:
        df_aux = df[df['name'] == pro][['amus', 'intensite']]
        df_final = df_final.merge(df_aux, how='left', left_on='mass', right_on='amus')
        df_final = df_final.drop('amus', 1)
        df_final = df_final.rename(columns={"intensite" : pro})
    
    df_final = df_final.rename(columns={"mass" : 'amus'})
    return df_final
...
def run_lasso_heuristique(date):
    list_profils = []
    seuil_last = np.inf
    best_result = ()
    
    profiles = pd.read_csv('./home/profiles.csv')
...
def compute_store_regressor(date, id_site, id_analysis):
    """
    compute lasso and odr result for one year and store them in a table
    """
    result_profils, result = run_lasso_heuristique(date)
...
# df_date = df_date[df_date['date'] >= (datetime.datetime.now()-timedelta(days=4))]

for value in df_date.values:
    date = value[0]
    print(date)
    compute_store_regressor(date, id_site, id_analysis)
    break
