...
def get_list_molecule_ids_by_ionization(ionization_mode, molecules):
    """
    ...
    L = [f"source = '{i}'" for i in molecules]
    sql = f"SELECT id_molecule FROM public.molecules where ion_polarity='{ionization_mode}' and ({' or '.join(L)});"
    connection = psycopg2.connect(user = USER, password = PASSWORD, host = HOSTNAME, port = PORT, database = DATABASE)
    df = sqlio.read_sql_query(sql, connection)
    connection.close()
    return(df.T.values[0])
...
####################################################################################
    ...
    pos_threshold = int(sys.argv[1])
    neg_threshold = int(sys.argv[2])
    sep = [i for i,j in enumerate(sys.argv) if j=='-v'][0]
    molecules_bdd=sys.argv[3:sep]
    print(molecules_bdd)
    MZML_FILES=sys.argv[sep+1:]
    MZML_FILES=[i.split('\\')[1] for i in MZML_FILES]
    print(MZML_FILES)
    for i in MZML_FILES:
        download_file(i)
        print(f"dowloaded : {i}")
    ...
    datetime_uplaod = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    add_2_analysis([id_analysis, datetime_uplaod, pos_threshold, neg_threshold, 0.003, 0.2, 'Null', 'Null'])
    print(id_analysis)
    for mzml_file in MZML_FILES:
        df_mzml = get_mzml_carac_from_db(mzml_file)
        id_mzml = df_mzml['id_mzml'].values[0]
        row = [id_analysis, id_mzml, mzml_file, datetime_uplaod, pos_threshold, neg_threshold, molecules_bdd]
        add_2_analysis_files(row)
    ...
    for i, mzml_file in enumerate(MZML_FILES):
        ...
        ionization_mode = df_mzml['ionisation'].values[0]
        ionization_mode = 'Positive' if ionization_mode=='POS' else 'Negative'
        molecule_ids = get_list_molecule_ids_by_ionization(ionization_mode, molecules_bdd)
        for id_molecule in molecule_ids:
            X,Y,Z = search_mass_in_mzml(mzml_file, id_molecule)
    ...
    for i, mzml_file in enumerate(MZML_FILES):
        ...
        ionization_mode_long = 'Positive' if ionization_mode=='POS' else 'Negative'
        acquisition_mode = df_mzml['acquisition_mode'].values[0]
        molecule_ids = get_list_molecule_ids_by_ionization(ionization_mode_long, molecules_bdd)
        # store maching between spec_id and retention time and energy collision of each spec
        run = pymzml.run.Reader(FILE_PATH_BASE + mzml_file)
        maching = []
        for n, spec in enumerate(run):
        ...
        for id_molecule in molecule_ids:
            ...
            ms_exp, X_peaks, Y_peaks, ids_spec_scan, ids_spec_frag_10, ids_spec_frag_20, ids_spec_frag_40 = get_extracted_peaks(mzml_file, acquisition_mode, ionization_mode, id_molecule, X=df_search_mass_in_mzml['time'].values, Y=df_search_mass_in_mzml['tic'].values, maching=maching, pos_threshold=pos_threshold, neg_threshold=neg_threshold)
            if len(ms_exp)==0 :
    ...
    for i, mzml_file in enumerate(MZML_FILES):
        ...
        for id_molecule, retention_time_exp, id_spec_scan in molecule_ids:
            ...
            print(df_isotopic_profile.head(1))
            # put df in the database
            engine = create_engine(f'postgresql://{USER}:{PASSWORD}@{HOSTNAME}:{PORT}/{DATABASE}')
    ...
    for i, mzml_file in enumerate(MZML_FILES):
        ...
        # Prepare data
        output = StringIO()
        d1.to_csv(output, sep='\t', header=False, index=False)
        output.seek(0)
        ...
        L2 = ['id_mzml', 'mzml_file', 'acquisition_mode', 'id_molecule', 'id_analysis', 'molecule_name', 'cas', 'formula', 'source', 'retention_time_exp','tic','mass_exp','cosine_similarity_10','cosine_similarity_20','cosine_similarity_40','root_similarity_10','root_similarity_20','root_similarity_40','scholle_similarity_10','scholle_similarity_20','scholle_similarity_40','num_fragmentation_the_peaks_10','num_fragmentation_the_peaks_20','num_fragmentation_the_peaks_40','num_fragmentation_exp_peaks_10','num_fragmentation_exp_peaks_20','num_fragmentation_exp_peaks_40','cosine_similarity_isotopic','root_similarity_isotopic','num_isotopic_the_peaks','num_isotopic_exp_peaks','cosine_similarity_isotopic_mod','retention_time','mass', 'flag_peak_number_exp_10', 'flag_peak_number_exp_20', 'flag_peak_number_exp_40', 'Validation']

        d['flag_peak_number_exp_10']=d['num_fragmentation_exp_peaks_10'].apply(lambda x : 'non' if x is None or x<=3 else 'oui')
        ...
        d['acquisition_mode'] = acquisition_mode
        d['acquisition_mode']=d['acquisition_mode'].apply(lambda x : 'Allion' if x=='Data independant' else 'MSMS')
        d['Validation'] = ''
        
        d = d[L2]

        LR_non28_3 = joblib.load('/home/LR_non28_3.pkl')
        LR_non28_3_ineris = joblib.load('/home/LR_non28_3_ineris.pkl')
        ...
        d = d[['id_mzml', 'id_molecule', 'id_analysis', 'mzml_file', 'molecule_name', 'cas', 'formula', 'mass', 'source', 'tic', 'retention_time_exp']+X1_ret[:1]+['flag_peak_number_exp_20']+X1_ret[1:3]+['flag_peak_number_exp_40']+X1_ret[3:]+['flag_peak_number_exp_10', 'cosine_similarity_10', 'scholle_similarity_10', 'Validation', 'acquisition_mode']]
        d[X1] = d[X1].fillna(0)
