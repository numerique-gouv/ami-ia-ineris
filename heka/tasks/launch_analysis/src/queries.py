...
def add_2_analysis_files(row):
    """
    Insert one record to public.analysis_files
    row : list containing all the values of columns of the record
    """
    sql = f"""INSERT INTO analysis_files 
    (id_analysis, id_mzml, mzml_file, date_analysis, threshold_positif, threshold_negatif, bdd) 
    VALUES({row[0]} , {row[1]} , '{row[2]}' ,' {row[3]}' , {row[4]}, {row[5]}, '["{'", "'.join(row[6])}"]');
    """
    run_query(sql)
